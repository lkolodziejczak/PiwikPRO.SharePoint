Param (
	...
	$FileName = 'installationLogsAzure' 
	$FolderName = '.logs'
...
$templateFilePath = "$scriptPath\templatesAzure\template.json";
$appPermissionsFilePath = "$scriptPath\templatesAzure\appPermissions.json";
...
$CertificatePassword = convertto-securestring "piwik123" -asplaintext -force
...
$cert | Export-PfxCertificate -FilePath "$($scriptPath)\PiwikPRO.pfx" -Password $CertificatePassword;
	...
	Write-Host "Trying to add admin consent permission... If there will be an error then you need to add it manually after the installation.";
start-sleep -s 5
