            ...
            ExecutionContext context)
        {
            ...
            if (req.Query["extension"] != StringValues.Empty)
            {
                foreach (string extension in req.Query["extension"])
                {
                    ...
                    {
                        monitoring.WriteLog("Storage", "GetConfig Storage exception - extension", exs.Message);
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(req.Query["tenant"]))
            {
                ...
                {
                    monitoring.WriteLog("Storage", "GetConfig Storage exception - tenant", exs.Message);
                }
            }

            req.HttpContext.Response.Headers["Cache-Control"] = "max-age=86400";

            return new OkObjectResult(Convert.ToString(piwikConfig));
        }
    }
}